...
public class Blowfish {
	// 4 S-box, with 256 entries of 32bits each, generated using PI hex digits
	static String Sbox[][] 
	...
	// 18 P-box of 32 bits each, initialized with PI hex digits
	static String Pbox[] 
	...
	static void generateKey(String key) throws Exception{
		...
		for(int i=0; i<18; i++) {
			String pbox = Converter.hexToBinary(Pbox[i]);
			
			while(pbox.length() < 32) {
				pbox = "0" + pbox;
			}
			
			String hex = Converter.binaryToHex(XOR(pbox, key.substring(j, j+32)));
	...
	static String[] splitInHalf(String bloc) throws Exception{
		...
		half[0] = bloc.substring(0, 32);
		half[1] = bloc.substring(32);
		
		return half;
	}
	
	static String groupBloc(String left, String right) throws Exception{
		if(left.length() != 32 && right.length() != 32) throw new Exception("Operands must be 32 bits of length");
		
		return left + right;
	}
	
	static String XOR(String s1, String s2) throws Exception{
		if(s1.length() != 32 && s2.length() != 32) throw new Exception("Operands must be 32 bits of length");
		
		StringBuffer res = new StringBuffer();
		
		for(int i=0; i<32; i++) {
			res.append(s1.charAt(i)^s2.charAt(i));
		}
		
		return res.toString();
	}

	static String F(String left) throws Exception{
		if(left.length() != 32) throw new Exception("Bloc must be 32 bits of length");
		
		String[] output = fourBytes(left);
		
		for(int i=0; i<4; i++) {
			output[i] = substitueByte(output[i], i);
		}
		
		String result = add(XOR(add(output[0], output[1]), output[2]), output[3]);
		
		return result;
	}
	
	static String[] fourBytes(String s) throws Exception{
		if(s.length() != 32) throw new Exception("Operand must be 32 bits of length");
		
		String[] bytes = new String[4];
		
		int j = 0;
		for(int i=0; i<4; i++) {
			bytes[i] = s.substring(j, j+8);
			j += 8;
		}
	...
	static String substitueByte(String b, int index) throws Exception{
		if(b.length() != 8) throw new Exception("Invalid length");
		
		int val = (int)Long.parseUnsignedLong(b, 2);
		String sbox = Converter.hexToBinary(Sbox[index][val]);
		
		while(sbox.length() < 32) {
			sbox = "0" + sbox;
		}
	...
	static String add(String b1, String b2) throws Exception{
		if(b1.length() != 32 && b2.length() != 32) throw new Exception("Operands must be 32 bits of length");
		long mod = (long) Math.pow(2, 32);
		
		long a = Converter.binaryToInteger(b1);
		long b = Converter.binaryToInteger(b2);
		
		Long r = (a+b) % mod;
		String res = Long.toBinaryString(r);
	...
	static String encryptRound(String bloc, int round) throws Exception{
		if(bloc.length() != 64) throw new Exception("Bloc must be 64 bits of length");
		
		String[] half = splitInHalf(bloc);
		String binPbox = Converter.hexToBinary(Pbox[round]);
		
		while(binPbox.length() < 32) {
			binPbox = "0" + binPbox;
		}
	...
	static String decryptRound(String bloc, int round) throws Exception{
		if(bloc.length() != 64) throw new Exception("Bloc must be 64 bits of length");
	...
	static String encryptAllRounds(String bloc) throws Exception{
		...
		for(int i=0; i<16; i++) {
			resBloc = encryptRound(resBloc, i);
		}
		
		return resBloc;
	}
	
	static String decryptAllRounds(String bloc) throws Exception{
		if(bloc.length() != 64) throw new Exception("Bloc must be 64 bits of length");
